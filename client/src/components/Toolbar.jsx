...
function Toolbar({tool, setTool, setStrokeWidth, setColour}) {
    ...
    return (
        <section className="toolbar">
            <button value={'pencil'} onClick={handleToolClick} className={tool==='pencil' ? 'active' : ''}>✏️</button>
            <button value={'pen'} onClick={handleToolClick} className={tool==='pen' ? 'active' : ''}>🖊️</button>
            <button value={'eraser'} onClick={handleToolClick} className={tool==='eraser' ? 'active' : ''}><BsEraserFill /></button>
            <span>|</span>
            <button onClick={togglePalette}>🎨</button>
            {showPalette && (
                <div className="paletteDropdown">
                    <input type="color" value={currentColour} onChange={handleColorChange} />
